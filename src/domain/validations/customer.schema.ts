...
const customerSchemaWithoutId = {
  ...
  phoneNumber: () =>
    z
      .string()
      .regex(
        /^(?:55)?(?:\(\d{2}\)|\d{2})\s?\d{5}-?\d{4}$/,
        'O número de telefone deve ser no padrão (XX) XXXXX-XXXX ou 55(XX) XXXXX-XXXX.'
      )
      .optional(),
  ...
  landlineNumber: () =>
    z
      .string()
      .regex(
        /^(?:55)?(?:\(\d{2}\)|\d{2})\s?\d{4}-?\d{4}$/,
        'O número de telefone fixo deve ser no padrão (XX) XXXX-XXXX ou 55(XX) XXXX-XXXX.'
      )
      .optional(),
