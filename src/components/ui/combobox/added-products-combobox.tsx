...
type AddedProductsComboboxProps = {
  selectedAddedProducts: WorkOrderResultItemInput[]
...
export function AddedProductsCombobox({
  ...
  const isProductAdded = (productId: string) => {
    return selectedAddedProducts.some((p) => p.productId === productId)
  }

  const getProductQuantity = (productId: string) => {
    const product = selectedAddedProducts.find((p) => p.productId === productId)
  ...
  const updateProductQuantity = (productId: string, quantity: number) => {
    if (quantity <= 0) {
      onAddedProductsChange(
        selectedAddedProducts.filter((p) => p.productId !== productId)
      )
    } else {
      onAddedProductsChange(
        selectedAddedProducts.map((p) =>
  ...
  const toggleProduct = (product: ProductWithCategoryDTO) => {
    if (isProductAdded(product.id)) {
      onAddedProductsChange(
        selectedAddedProducts.filter((p) => p.productId !== product.id)
      )
    } else {
      onAddedProductsChange([
        ...selectedAddedProducts,
  ...
  const selectedProductsText = useMemo(() => {
    if (selectedAddedProducts.length === 0) return placeholder
    if (selectedAddedProducts.length === 1) {
      const qty = selectedAddedProducts[0].quantity
      const name = selectedAddedProducts[0].productName
      return `${name}${qty > 1 ? ` (${qty}x)` : ''}`
    }
    const totalQty = selectedAddedProducts.reduce(
      (sum, p) => sum + p.quantity,
      0
    )
    return `${selectedAddedProducts.length} produtos (${totalQty} itens)`
  }, [selectedAddedProducts, placeholder])

  const renderProductItem = ({ item }: { item: ProductWithCategoryDTO }) => {
    const selected = isProductAdded(item.id)
  ...
  const hasValue = selectedAddedProducts.length > 0
